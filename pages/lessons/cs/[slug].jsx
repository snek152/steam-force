		...
		<Layout title={data.title} container={false}>
			...
			<div className="flex max-w-7xl sm:flex-row flex-col mx-auto py-6 px-4 sm:px-6 lg:px-8 relative flex-grow">
				...
				<div className="w-full inline-block prose prose-pre:bg-[#22272e] prose-pre:p-0 prose-h2:border-t-2 prose-h2:pt-5 border p-4 shadow-lg rounded-2xl border-opacity-80 bg-white max-w-none sm:w-9/12">
					...
					>
						<div
							className={`bg-gray-50 shadow-md rounded-md p-3 mb-5 ${
								user.completed?.includes(data.slug)
									? "bg-white text-gray-400 cursor-not-allowed pointer-events-none"
									: ""
							}`}
						>
							...
							<form onSubmit={formSubmit}>
								...
								<button
									type="submit"
									className={`text-white py-1 px-2 rounded-md shadow-lg ${
										user.completed?.includes(data.slug)
											? "bg-blue-200"
											: "bg-blue-500"
									}`}
					...
					<div className="flex justify-between h-10">
						<Link href={`/lessons/cs/${data.prev}`}>
							<a
								className={`bg-gray-300 rounded-md shadow-md no-underline pt-[0.4rem] px-3 font-light ${
									data.prev
										? ""
						...
						<Link href={`/lessons/cs/${data.next}`}>
							<a
								className={`bg-blue-500 rounded-md shadow-md no-underline text-white pt-[0.4rem] px-3 font-light ${
									data.next
										? ""
										: "pointer-events-none bg-blue-300 text-gray-100"
								}`}
							>
								Next
							</a>
